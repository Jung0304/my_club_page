...
window.selectOnly = function (btn) {
...
function showClubBox(text, intro, targetId = "content-box") {
    ...
    const dimmedClass = !isRecruitOpen ? "dimmed" : "";
    const overlay = !isRecruitOpen
      ? `<div class="card-overlay">
           <div class="overlay-message">지금은 신청을 받고 있지 않습니다.</div>
         </div>`
      : "";
  
    box.innerHTML += `
      <div class="inner-card ${dimmedClass}">
        ${overlay}
        <h3>${text}</h3>
        <p>${intro}</p>
        <div class="card-buttons">
            <button ${!isRecruitOpen ? "disabled" : ""} onclick="alert('폼으로 이동')">신청하기</button>
        </div>
      </div>
    `;
  }
